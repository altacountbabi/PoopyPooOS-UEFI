...
pub fn decode_png(raw_data: Vec<u8>) -> PngData {
    ...
    for chunk in raw_pixels.chunks_exact(4) {
        let alpha = alpha_blend(
            Color::new(chunk[0], chunk[1], chunk[2]),
            Color::new(0, 0, 0),
            chunk[3],
        );
        pixels.push(alpha);
...
fn alpha_blend(source_color: Color, dest_color: Color, alpha: u8) -> Color {
    ...
    let result_red = (f32::from(source_color.red) * alpha_normalized)
        + (f32::from(dest_color.red) * (1.0 - alpha_normalized));
    let result_green = (f32::from(source_color.green) * alpha_normalized)
        + (f32::from(dest_color.green) * (1.0 - alpha_normalized));
    let result_blue = (f32::from(source_color.blue) * alpha_normalized)
        + (f32::from(dest_color.blue) * (1.0 - alpha_normalized));
